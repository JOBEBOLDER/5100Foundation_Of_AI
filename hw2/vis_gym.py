...
def setup(GUI=True):
    global screen, clock
    if GUI:
        # Robust initialization for macOS/window focus quirks.
...
def refresh(obs, reward, done, info, delay=0.1):
    ...
    # Safety: if caller expects GUI but screen wasn't created, initialize now.
    if screen is None:
        setup(GUI=True)

    # Keep window responsive and allow close button handling.
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
